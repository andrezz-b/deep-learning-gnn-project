...
class SemiSupervisedEnsemble:
    ...
    def _forward(self, model, batch, prefer_teacher: bool = False):
        if prefer_teacher:
            use_teacher = getattr(model, "teacher", None) is not None #checks if the model has an attribute called teacher
            if use_teacher:
                try:
    ...
    def validate(self):
        ...
        with torch.no_grad():
            for x, targets in self.val_dataloader:
                ...
                # Ensemble prediction
                preds = [self._forward(model, x, prefer_teacher=True) for model in self.models]
    ...
    def train(self, total_epochs, validation_interval):
        ...
        for epoch in (pbar := tqdm(range(1, total_epochs + 1))):
            ...
            if self.unsupervised_train_dataloader is not None and self.unsupervised_loss_weight > 0:
                unsupervised_iterator = iter(self.unsupervised_train_dataloader)
            for x, targets in self.train_dataloader:
                x, targets = x.to(self.device), targets.to(self.device)
                self.optimizer.zero_grad()
                # Supervised loss
                supervised_losses = [self.supervised_criterion(self._forward(model, x), targets) for model in self.models]
                ...
                if unsupervised_iterator is not None:
                    ...
                    for model in self.models:
                        ...
                        consistency_terms.append(torch.nn.functional.mse_loss(student_preds, teacher_preds.detach()))
